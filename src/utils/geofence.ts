...
// src/utils/geofence.ts - DYNAMIC HAVERSINE WITH ACCURACY TOLERANCE

export interface GeofenceValidationResult {
  isWithinBounds: boolean;
  actualDistanceMeters: number;
  effectiveRadiusMeters: number;
  gpsAccuracyError: number;
}
...
export const verifyPunchLocationWithHardwareTolerance = (
  userLat: number,
  userLng: number,
  hospitalLat: number,
  hospitalLng: number,
  configuredRadiusMeters: number = 150,
  gpsAccuracyErrorMeters: number = 0
): GeofenceValidationResult => {
  const distance = calculateHaversineDistance(userLat, userLng, hospitalLat, hospitalLng);

  // Dynamic Tolerance: Grant 50% of GPS hardware uncertainty up to 80m max
  const hardwareToleranceBuffer = Math.min(gpsAccuracyErrorMeters * 0.5, 80);
  const effectiveRadius = configuredRadiusMeters + hardwareToleranceBuffer;

  return {
    isWithinBounds: distance <= effectiveRadius,
    actualDistanceMeters: distance,
    effectiveRadiusMeters: effectiveRadius,
    gpsAccuracyError: gpsAccuracyErrorMeters
  };
};
